...
public static class EventDelegateEditor
{
	...
	static public bool Field (Object undoObject, EventDelegate del, bool removeButton, bool minimalistic)
	{
		...
		if (remove)
		{
            CustomEditorTools.RegisterUndo("Delegate Selection", undoObject);
			del.Clear();
			...
			if (ps != null)
			{
				for (int i = 0; i < ps.Length; ++i)
				{
					...
					if (selGO != null)
					{
						...
						int selection;
                        string[] props = GetNames(ents, CustomEditorTools.GetFuncName(param.obj, param.field), out selection);
	...
	static public string[] GetNames (List<Entry> list, string choice, out int index)
	{
		...
		{
			Entry ent = list[i];
            string del = CustomEditorTools.GetFuncName(ent.target, ent.name);
			names[++i] = del;
