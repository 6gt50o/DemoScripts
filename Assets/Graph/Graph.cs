...
class GraphVertex
{
    ...
    public GraphVertex(string Vertexdata)
    {
        Data = Vertexdata;
    }
}

public class GraphCtor
{
    //图中所能包含的点上限
    private const int Number = 10;
    //顶点数组
    private GraphVertex[] vertiexes;
    //邻接矩阵
    ...
    public GraphCtor()
    {
        ...
        vertiexes = new GraphVertex[Number];
        //将代表邻接矩阵的表全初始化为0
    ...
    //向图中添加节点
    public void AddGraphVertex(string v)
    {
        vertiexes[numVerts] = new GraphVertex(v);
        numVerts++;
    }
    //向图中添加有向边
    ...
    {
        Debug.Log(vertiexes[vertexPosition].Data + " ");
    }
    ...
    public void TopSort()
    {
        ...
        Stack<GraphVertex> sealVerts = new Stack<GraphVertex>();
...
public class Graph : MonoBehaviour {
	...
	void Start () {
        /*GraphCtor g = new GraphCtor();
        g.AddGraphVertex("A");
        g.AddGraphVertex("B");
        g.AddGraphVertex("C");
        g.AddGraphVertex("D");
        g.AddEdge(0, 1);//1 2 3 4  <11 12 13 14 == 21 22 23 24 == 31 32 33 34 == 41 42 43 44>
        g.AddEdge(1, 2);//2 
        g.AddEdge(2, 3);//3
        g.AddEdge(3, 4);//4
        g.TopSort();*/
